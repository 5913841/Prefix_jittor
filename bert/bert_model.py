...
class BertConfig(AutoConfig, PretrainedConfig):
    """
...
class BertClassification(BertBase):
    def __init__(self, config):
        ...
        self.bert = BertModelWithPrefix(config)
...
class BertModelWithPrefix(BertBase):

    def __init__(self, config, add_pooling_layer=True):
        super().__init__(config)
        self.embeddings = BertEmbeddings(config)
        self.encoder = BertEncoderWithPrefix(config)
...
class BertEncoderWithPrefix(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.layer = nn.ModuleList([BertLayer(config) for _ in range(config.num_hidden_layers)])
    ...
    def execute(
        ...
        if attention_mask is None:
            attention_mask = jittor.ones(input_shape[:2])
        
        if prefix is not None:
            attention_mask = jittor.concat((jittor.ones(prefix[0].shape[:2], attention_mask.dtype), attention_mask), dim = 1)
        ...
        if prefix is None:
            ...
            for i, layer_module in enumerate(self.layer):
                ths_prefix = prefix[i]
                # print(ths_prefix[0,3,4])
                if i == 0:
                    hidden_states = jittor.concat((ths_prefix,hidden_states), dim = 1)
                else:
                    hidden_states[:,:ths_prefix.shape[1],:] = ths_prefix
            ...
            return {
                "last_hidden_state": hidden_states[:,ths_prefix.shape[1]:,:],
            }
...
class BertForQuestionAnswering(BertBase, PreTrainedModel):
    config_class = BertConfig
    base_model_prefix = "transformer"

    def __init__(self, config):
        BertBase.__init__(self,config)
        self.num_labels = config.num_labels

        self.bert = BertModelWithPrefix(config, add_pooling_layer=False)
