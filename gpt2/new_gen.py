...
MODEL_CLASSES = {"gpt2": (GPT2LMHeadModel, GPT2Tokenizer)}  # waiting for jittor implementation
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        required=False,
        help="Path to pre-trained PrefixTuning Model or shortcut name selected in the list: " + ", ".join(
            MODEL_CLASSES.keys()),
    )
    ...
    if args.task_mode == 'data2text':
        ...
        if QUICK_CHECK:
            ...
            if ('lowdata' in args.model_name_or_path) or (
                    args.prefixModel_name_or_path is not None and 'lowdata' in args.prefixModel_name_or_path):
                test_path = '../data/e2e_data/src1_valid.txt'  # 低数据模式
            else:
    ...
    for prompt_idx, prompt_text in enumerate(prompt_text_lst):  # prompt_text就是输入的表格
        ...
        if args.control_mode == 'yes' and args.control_dataless != 'yes':
            ...
            # control_code = jittor.int64(control_codes[prompt_idx]).to(model.device).unsqueeze(0).expand(args.num_return_sequences, -1)
            control_code = None
            pass
        else:
            control_code = None
        ...
        if args.tuning_mode == 'prefixtune' or args.tuning_mode == 'bothtune':
            if args.task_mode == 'data2text':
                ...
                src_cat = tokenizer(cat, add_special_tokens=True, truncation=True, is_split_into_words=True)[
                    'input_ids']
                ...
                if mode in ['cat', 'peek', 'nopeek', 'infix']:
                    control_code = jt.int64(cc).to(model.device).unsqueeze(0)
                elif mode == 'instruction_based':
                    control_code = None
                else:
                    assert False, "invalid mode type."
            ...
            if args.format_mode != 'infix':
                ...
                print(control_code)
                print(src)
                src = jt.int64(src).to(model.device).unsqueeze(0)
                print(input_ids)
                prompt = model.get_prompt(src, None, gpt2=gpt2,
                                          bsz=1)  # src, control_code=None, gpt2=None, bsz=None, attn_mask=None
